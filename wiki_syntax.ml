...
let translate_link ~oldwiki ~newwiki ~newwikipath addr frag attribs (_sp, wb) =
  (Lwt.return Int32.zero) >>= fun currentwiki ->
...
let simple_builder =
  let void_plugin = fun _ -> (false, (fun a b c -> Wikicreole.A_content (Lwt.return {{ "" }} ))) in
...
let std_of_wiki bi s  =
  Wikicreole.from_string bi simple_builder s >>= function
      | [] -> Lwt.return {{ [] }}
      | a::_ -> a
